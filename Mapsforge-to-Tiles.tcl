...
pack .title -pady {0 3}
hyperlink .title https://github.com/JFritzle/Mapsforge-to-Tiles
...
pack .maps_folder -in .l -fill x -pady 1
...
pack .maps.values -side left -fill both -expand 1
...
pack .themes -in .l -fill x -pady 1
...
pack .overlays_show_hide -in .styles -fill x -pady {2 0}
...
pack .shading.onmap .shading.asmap -fill x
...
pack .server.interface -fill x -pady {4 2}
...
proc update_theme_styles_overlays {} {
  ...
  pack configure .styles -in .l -after .themes -fill x -pady 1
  resize_toplevel_window .
}
...
proc update_overlays_selection {} {
  ...
  pack $parent.label $parent.separator1 -fill x -pady {0 2}
  set overlays [lindex $style 2]
  foreach overlay $overlays {
    set overlay_id [lindex $overlay 0]
...
proc pipe_run {exe args} {
  ...
  if {[catch {open "| $cmd 2>@1" r} result]} \
...
proc curl_event {fd fdlog} {
  ...
  while {[gets $fd line] >= 0} {
	...
	set pipe::abort 1
      ...
      #  lappend text "> Get $url Status $status Size $size bytes Time $time ms"
      }
    }
  }
  if {$text != {}} {cputs [join $text \n]}
...
# cputs [get_shell_command $curl_exec]
  if {[catch {open "| $curl_exec" r} result]} {
...
proc run_render_job {} {
  ...
  if {![file exists $file]} return
  switch $::tcl_platform(os) {
    "Windows NT" {set exec "exec cmd.exe /C START {} \"$file\""}
    "Linux"	 {set exec "exec nohup xdg-open \"$file\" >/dev/null"}
    "Darwin"	 {set exec "exec nohup open \"$file\" >/dev/null"}
  }
  after 0 "catch {$exec}"
  return
